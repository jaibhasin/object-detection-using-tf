...
OUTPUT_WITH_AUDIO = "voice993_final.mp4"
# Process depth estimation every N frames for speed
DEPTH_FRAME_SKIP = 10  # Increased for faster processing
# ------------------------
# NAVIGATION ZONES
# ------------------------
...
# ------------------------
ALERT_COOLDOWN = 3.0 # Seconds between alerts for same object type (video time)
...
class VoiceAlert:
    def __init__(self):
        self.last_alert_time = {}  # Now stores video time
        self.speaking = False
        self.audio_clips = [] # Store (timestamp, audio_file) tuples
        self.fps = 30
        self.temp_dir = tempfile.mkdtemp()
       
    def should_alert(self, object_type, depth_value, frame_number):
        """Check if we should alert about this object (video time cooldown)"""
        if depth_value < MIN_ALERT_DISTANCE:
            return False
           
        video_time = frame_number / self.fps
        key = object_type  # Cooldown per object type only
       
        if key in self.last_alert_time:
            time_since_last = video_time - self.last_alert_time[key]
            if time_since_last < ALERT_COOLDOWN:
                return False
       
        self.last_alert_time[key] = video_time
        return True
    ...
    def speak_and_record(self, message, frame_number):
        ...
        try:
            ...
            # Calculate timestamp in seconds (video time)
...
def main():
    ...
    def draw_zones(frame):
        ...
        cv2.putText(frame, "USER PATH", (left_line + 10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)
    ...
    while True:
        ...
        if center_zone_objects:
            ...
            for obj in center_zone_objects[:1]:
                ...
                if alert_msg and voice_alert.should_alert(obj['label'], obj['depth'], frame_count):
